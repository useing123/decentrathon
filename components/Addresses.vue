            ...
            <table class="w-full border">
                ...
                <tbody>
                    <tr v-for="(address, index) in addresses" :key="index" class="border-b">
                        <td class="py-2 px-4"><a :href="getAddressUrl(address)">{{ address.address }}</a></td>
...
export default {
    ...
    methods: {
        async fetchAddresses() {
            try {
                const response = await api.get('/api?module=account&action=balancelist');
        ...
        getAddressUrl(address) {
            return `https://bscscan.com/address/${address.address}`;
        },
    },
    created() {
        this.fetchAddresses();
